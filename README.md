...
> 给 Claude Code 一条跨会话的「我让它做过什么」流水 —— 纯 hook，零依赖，不烧 token。
...
它不是知识库，也不是 RAG。它记的是一条**时间线**：
「`HH:MM 问：…` → `↳ 答：…`」，过滤掉寒暄，按月归档，定期语义压缩。
...
- 记的是精炼问答流水（已过滤寒暄），不是整段 transcript
- 压缩是 skill 驱动的**语义浓缩**（保留脉络），不是机械截断
